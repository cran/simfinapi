...
test_that("sfa_load_companies works with details = FALSE/TRUE", {
    ...
    for (details in c(FALSE, TRUE)) {
        ...
        expect_data_table(entities, key = "id", types = c("integer", "character"), min.rows = 4000L, min.cols = 7L, max.cols = 11L,
            col.names = "strict")
...
test_that("sfa_load_companies returns error if api key is incorrect", {
    expect_error(sfa_load_companies("invalid_api_key"), "SimFin API Error 401: Invalid API Key - check the key again and also if you confirmed your e-mail on registration",
        fixed = TRUE)
})
